...
{
 $query .= 'WHERE vsd_ID LIKE "%'.$_POST["search"]["value"].'%" ';
...
foreach($result as $row)
{
	...
	if(empty($row["user_ID"]))
	{
		...
		$acreg = "UN";
		$btnrg = '<button type="button" class="btn btn-warning btn-sm gen_account" data-toggle="tooltip" data-html="true" title="Generate Account" id="'.$row["vsd_ID"].'">
	...
	$sub_array = array();
		...
		$sub_array[] = '
		<div class="btn-group">
		  <button class="btn btn-info btn-sm view"  id="'.$row["vsd_ID"].'"><i class="icon-eye" style="font-size: 20px;"></i></button>
		  <button class="btn btn-primary btn-sm edit"  id="'.$row["vsd_ID"].'"><i class="icon-database-edit2" style="font-size: 20px;"></i></button>
		  '.$btnrg.'
		</div>
		';
		// <div class="dropdown-divider"></div>
		 // <a class="dropdown-item delete" id="'.$row["vsd_ID"].'">Delete</a>
	$data[] = $sub_array;
}
