...
`rad`.`rad_SchID`,
`rad`.`rad_FName`,
LEFT(`rad`.`rad_MName`,1) rad_MName,
`rad`.`rad_LName`,
`rad`.`user_ID`,
`rs`.`sex_Name`,
`rm`.`marital_Name`,
...
if(isset($_POST["search"]["value"]))
{
 $query .= 'WHERE rad_ID LIKE "%'.$_POST["search"]["value"].'%" ';
    $query .= 'OR rad_SchID LIKE "%'.$_POST["search"]["value"].'%" ';
...
foreach($result as $row)
{
	...
	if(empty($row["user_ID"]))
	{
		...
		$acreg = "UN";
		$btnrg = '<button type="button" class="btn btn-warning btn-sm gen_account" data-toggle="tooltip" data-html="true" title="Generate Account" id="'.$row["rad_ID"].'">
	...
	for($ds=1;$ds<=$cc_ID ;$ds++){
		...
		$sub_array[] =  $add_zeroinID.$row["rad_SchID"];
		$sub_array[] =  ucwords(strtolower($row["rad_LName"].', '.$row["rad_FName"].' '.$mname.' '.$suffix));
		$sub_array[] =  ucwords(strtolower($row["sex_Name"]));
		$sub_array[] =  ucwords(strtolower($row["marital_Name"]));
		$sub_array[] =  $reg;
		$sub_array[] = '
		<div class="btn-group">
		  <button class="btn btn-info btn-sm view"  id="'.$row["rad_ID"].'"><i class="icon-eye" style="font-size: 20px;"></i></button>
		  <button class="btn btn-primary btn-sm edit"  id="'.$row["rad_ID"].'"><i class="icon-database-edit2" style="font-size: 20px;"></i></button>
		  '.$btnrg.'
		</div>
		';
