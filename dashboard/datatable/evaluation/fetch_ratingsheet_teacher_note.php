...
ua.lvl_ID
";
$query .= "FROM `observation_notes`  obn
LEFT JOIN ref_period  rp ON rp.period_ID = obn.period_ID
LEFT JOIN ref_semester rsem ON rsem.sem_ID = obn.sem_ID
...
if (isset($_REQUEST["staff_ID"])){
	$query .= ' WHERE obn.acs_ID =  '.$_REQUEST["staff_ID"]." AND ua.lvl_ID = 3 AND ";
}
...
{
 $query .= '(obn.obn_ID LIKE "%'.$_POST["search"]["value"].'%" ';
    $query .= 'OR rp.period_Name LIKE "%'.$_POST["search"]["value"].'%" ';
    $query .= 'OR obn.obn_Date LIKE "%'.$_POST["search"]["value"].'%" )';
}
...
foreach($result as $row)
{
	...
	$sub_array = array();
		...
		$sub_array[] = $row["obn_ID"];
		$sub_array[] = $row["Observer_Name"];
		$sub_array[] = $row["obn_Date"];
		$sub_array[] = $row["period_Name"];
		$sub_array[] = '
		<div class="btn-group">
		  <a class="btn btn-primary btn-sm view" href="print/print?report=rating-sheet-note&obn_ID='.$row["obn_ID"].'" target="_BLANK"  id="'.$row["obn_ID"].'"><i class="icon-printer" style="font-size: 20px;"></i></a>
		</div>
		';
	$data[] = $sub_array;
}
