...
?>
        <script type="text/javascript">
          ...
          $(document).on('submit', '#change_password_form', function(event){
            ...
            $(document).on('click', '.acc_deactive', function(){
               var acc_ID = $(this).attr("id");

               alertify.confirm('Are you sure you want to activate this  account?', 
            function(){
              $.ajax({
               type        :   'POST',
               url:"datatable/account/insert.php",
               data        :   {operation:"deactivate_acc",acc_ID:acc_ID},
               dataType    :   'json',
               complete     :   function(data) {
                
                 alertify.alert(data.responseText).setHeader('Deactivate Account');
                 admin_dataTable.ajax.reload();
            ...
            }).setHeader('Deactivate Account');
               
            

            });

            $(document).on('click', '.acc_active', function(){
            ...
            function(){
              $.ajax({
               type        :   'POST',
               url:"datatable/account/insert.php",
               data        :   {operation:"activate_acc",acc_ID:acc_ID},
               dataType    :   'json',
               complete     :   function(data) {
                
                 alertify.alert(data.responseText).setHeader('Activate Account');
                 admin_dataTable.ajax.reload();
