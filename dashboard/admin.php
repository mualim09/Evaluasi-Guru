    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
      ...
      <nav aria-label="breadcrumb" >
        <ol class="breadcrumb bcrum">
          ...
          <li class="breadcrumb-item  active bcrum_i_ac" aria-current="page">Admin Record Management</li>
        </ol>
      </nav>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" id="product_modal_content">
            <div class="form-row">
                ...
                <div class="form-group col-md-4">
                  <label for="admin_schID">Government ID<span class="text-danger">*</span></label>
                  <input type="text" class="form-control" id="admin_schID" name="admin_schID" placeholder="" value="" onkeypress="return isNumberKey(event)" maxlength="7" required="">
                </div>
...
?>
        <script type="text/javascript">
         ...
         $(document).on('onkeyup change', $("#admin_schID"), function(el) {
          // console.log($(el.target).val());
          pad($(el.target).val(),7,"#admin_schID");
          });
          ...
          $(document).on('submit', '#admin_form', function(event){
            event.preventDefault();
              ...
              $.ajax({
                ...
                {
                  alertify.alert(data).setHeader('Admin Record');
                  $('#admin_form')[0].reset();
          ...
          $(document).on('click', '.add', function(){
            $('#admin_modal_title').text('Add New Admin');
            ...
            $("#admin_schID").prop("disabled", false);
          ...
          $(document).on('click', '.view', function(){
            ...
            $('#admin_modal_title').text('View Admin');
            $('#admin_modal').modal('show');
            ...
            // btng.classList.add("btng_null");
             ...
             $.ajax({
                ...
                method:'POST',
                data:{action:"admin_view",admin_ID:admin_ID},
                dataType    :   'json',
                success:function(data)
                {
                  $("#admin_schID").prop("disabled", true);
                  ...
                  $('#admin_schID').val(data.admin_schID);
            ...
            var admin_ID = $(this).attr("id");
            $('#admin_modal_title').text('Edit Admin');
            $('#admin_modal').modal('show');
            ...
            // btng.classList.add("btn-group");
             ...
             $.ajax({
                url:"datatable/admin/fetch_single.php",
                method:'POST',
                data:{action:"admin_update",admin_ID:admin_ID},
                dataType    :   'json',
                success:function(data)
                {
                  $("#admin_schID").prop("disabled", false);
          ...
          $(document).on('click', '#admin_delform', function(event){
            ...
            $.ajax({
             type        :   'POST',
             url:"datatable/admin/insert.php",
             data        :   {operation:"delete_admin",admin_ID:admin_ID},
             dataType    :   'json',
             complete     :   function(data) {
               $('#deladmin_modal').modal('hide');
               alertify.alert(data.responseText).setHeader('Delete this Admin');
               admin_dataTable.ajax.reload();
          ...
          $(document).on('click', '.gen_account', function(event){
            ...
            function(){
              $.ajax({
               type        :   'POST',
               url:"datatable/admin/insert.php",
               data        :   {operation:"gen_account",admin_ID:admin_ID},
               dataType    :   'json',
               complete     :   function(data) {
